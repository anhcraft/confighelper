...
public class ConfigDoc {
    ...
    public ConfigDoc generate(@NotNull File output){
        ...
        for (ConfigSchema<?> schema : schemas){
            ...
            StringBuilder confBuilder = new StringBuilder("<!Doctype html><html><head><title>Config | ").append(title).append("</title><meta charset=\"UTF-8\"/><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"/><meta name=\"robots\" content=\"none\"/><style type=\"text/css\">body{font-family: monospace; font-size: 16px;}a{text-decoration: none; color: #19afdc;}table{width: 100%;}table th{font-size: 18px; background-color: #63c37c;}table, table td, table th{border: 1px solid #555; border-collapse: collapse; padding: 7px 12px;}table tr:hover{background: #eee;}header{padding: 15px 50px;}#container{display: grid; grid-template-columns: auto auto auto auto auto auto;grid-gap: 20px;}#right-side{grid-column: 2 / 6;}#left-side{border-right: 1px #e6e6e6 solid;padding: 0 20px;font-size: 16px;text-transform: lowercase;}footer{position: fixed; left: 0; bottom: 0; background: #f2d9ff; width: 100%; padding: 5px; text-align: center;}</style></head><body><header><a href=\"index.html\">Overview</a> | <a href=\"javascript:window.history.back()\">Back</a><br/><br/><h1>").append(title).append("</h1></header><div id=\"container\"><div id=\"left-side\">").append(sideMenuStr).append("</iframe></div><div id=\"right-side\"><table><tr><th>Key</th><th>Type</th><th>Explanation</th></tr>");
