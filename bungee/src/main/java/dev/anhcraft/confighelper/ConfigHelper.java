...
public class ConfigHelper {
    ...
    public static <T> T readConfig(@NotNull Configuration bungeeConf, @NotNull ConfigSchema<T> configSchema, @Nullable T object) throws InvalidValueException {
        ...
        for (String k : configSchema.listKeys()){
            ...
            Object value = bungeeConf.get(k);
            value = configSchema.callMiddleware(entry, value, object, Middleware.Direction.CONFIG_TO_SCHEMA);

            if(entry.getValueSchema() != null && value instanceof Configuration){
                value = readConfig((Configuration) value, entry.getValueSchema());
            } else {
                ...
                if(value != null && entry.getComponentClass() != null){
                    if(List.class.isAssignableFrom(field.getType())){
                        ...
                        if(!list.isEmpty()){
                            ...
                            for(Object o : list){
                                if(entry.getValueSchema() != null && o instanceof Configuration){
                                    nlist.add(readConfig((Configuration) o, entry.getValueSchema()));
                        ...
                        if(len > 0){
                            ...
                            for(int i = 0; i < len; i++){
                                ...
                                if(entry.getValueSchema() != null && o instanceof Configuration){
                                    Array.set(arr, i, readConfig((Configuration) o, entry.getValueSchema()));
                                } else if(entry.isPrettyEnum() && o instanceof String) {
